...
@torch.no_grad()
def check_decoder_norms(sae_or_W_dec) -> bool:
    """
    Check that decoder directions are approximately unit-normalized.
    
    Interface-based detection (extensible to any decoder type):
    1. If SAE has get_decoder_directions(): use it, warn if not unit-norm
    2. Elif SAE has W_dec (2D tensor): check raw W_dec, raise error if not normalized
    3. Else: warn and skip
    
    Args:
        sae_or_W_dec: Either an SAE object or a W_dec tensor
        
    Returns:
        True if norms are approximately unit, False otherwise
        
    Raises:
        ValueError if SAE has W_dec but no get_decoder_directions and W_dec is not normalized
    """
    import warnings
    
    # Handle raw tensor (backwards compatibility)
    if isinstance(sae_or_W_dec, torch.Tensor):
        W_dec = sae_or_W_dec
        source = "raw_tensor"
    # Interface-based: prefer get_decoder_directions() if available
    elif hasattr(sae_or_W_dec, 'get_decoder_directions') and callable(sae_or_W_dec.get_decoder_directions):
        W_dec = sae_or_W_dec.get_decoder_directions()
        source = "get_decoder_directions"
    # Fallback: use raw W_dec for legacy SAEs without get_decoder_directions
    elif hasattr(sae_or_W_dec, 'W_dec') and sae_or_W_dec.W_dec.ndim == 2:
        W_dec = sae_or_W_dec.W_dec
        source = "raw_W_dec"
    else:
        warnings.warn("SAE has no W_dec or get_decoder_directions method. Skipping decoder norm check.")
        return False
    
    # Check norms along feature dimension (rows for shape (d_sae, d_in))
    ...
    if torch.allclose(norms, torch.ones_like(norms), atol=tolerance):
        ...
        msg = f"Decoder directions are not normalized. Max diff: {max_diff.item()}"
        
        if source == "get_decoder_directions":
            # SAE provided directions via interface but they're not unit-norm
            # This is unexpected but not fatal - warn and continue
            warnings.warn(msg + " (from get_decoder_directions)")
            return False
        elif source == "raw_W_dec":
            # Legacy SAE without get_decoder_directions - raise error for backwards compat
            print(msg)
            raise ValueError(
                "Decoder weights are not normalized. Refer to base_sae.py and relu_sae.py for more info."
            )
        else:
            # Raw tensor passed directly
            print(msg)
            raise ValueError("Decoder tensor is not normalized.")
...
def load_and_format_sae(
    ...
    if isinstance(sae_object_or_sae_lens_id, str):
        ...
        sae_id = "custom_sae"
        sparsity = None
        check_decoder_norms(sae)  # Pass full SAE object for get_decoder_directions() support
