...
@beartype
def compute_mean_wasserstein_distance(
    acts_BD: torch.Tensor,
    labels_B: torch.Tensor,
) -> float:
    """
    Compute the mean Wasserstein distance between positive and negative class 
    activation distributions across all features.
    
    For each feature dimension, computes the 1D Wasserstein distance between
    the positive and negative class distributions, then averages across features.
    """
    positive_mask_B = labels_B == dataset_info.POSITIVE_CLASS_LABEL
    negative_mask_B = labels_B == dataset_info.NEGATIVE_CLASS_LABEL
    
    positive_acts = acts_BD[positive_mask_B].cpu().numpy()
    negative_acts = acts_BD[negative_mask_B].cpu().numpy()
    
    if len(positive_acts) == 0 or len(negative_acts) == 0:
        return 0.0
    
    # Compute Wasserstein distance for each feature dimension
    num_features = acts_BD.shape[1]
    distances = []
    
    for d in range(num_features):
        # Get the 1D distributions for this feature
        pos_dist = positive_acts[:, d]
        neg_dist = negative_acts[:, d]
        
        # Compute 1D Wasserstein distance
        dist = wasserstein_distance(pos_dist, neg_dist)
        distances.append(dist)
    
    # Return mean Wasserstein distance across features
...
def train_sklearn_probe(
    ...
    """Train a logistic regression probe and return the probe, test accuracy, and test F1 score."""
    ...
    # Get predictions
    test_predictions = probe.predict(test_inputs_np)

    # Compute accuracies and F1 scores
    ...
    test_f1 = f1_score(test_labels_np, test_predictions, average='binary')

    if verbose:
        print("\nTraining completed.")
        print(f"Train accuracy: {train_accuracy}, Test accuracy: {test_accuracy}, Test F1: {test_f1}\n")

    return probe, test_accuracy, test_f1
...
def test_probe_gpu(
    ...
    """Test probe and return (accuracy, f1_score)."""
...
def train_probe_gpu(
    ...
    Returns: (best_probe, best_test_accuracy, best_test_f1)"""
    ...
    for epoch in range(epochs):
        ...
        train_accuracy, train_f1 = test_probe_gpu(train_inputs, train_labels, batch_size, probe)
        test_accuracy, test_f1 = test_probe_gpu(test_inputs, test_labels, batch_size, probe)
        ...
        if verbose:
            print(
                f"Epoch {epoch + 1}/{epochs} Loss: {loss.item()}, train accuracy: {train_accuracy}, test accuracy: {test_accuracy}, test F1: {test_f1}"  # type: ignore
            )
    ...
    return best_probe, best_test_accuracy, best_test_f1
...
def train_probe_on_activations(
...
) -> tuple[dict[str, LogisticRegression | Probe], dict[str, float], dict[str, float], dict[str, float]]:
    """Train a probe on the given activations and return the probe and metrics for each profession.
    ...
    Wasserstein distance is computed on the FULL activations, before any top-k mask is applied,
    so it does not depend on `select_top_k`. Pass `compute_wasserstein=False` for the redundant
    higher-k calls in callers that already report Wasserstein from the k=1 (or full) pass.

    Returns:
        probes: dict of trained probes per profession
        test_accuracies: dict of test accuracy per profession
        test_f1_scores: dict of test F1 score per profession
        wasserstein_distances: dict of mean Wasserstein distance per profession (empty when
            compute_wasserstein=False)
    """
    torch.set_grad_enabled(True)

    probes, test_accuracies, test_f1_scores, wasserstein_distances = {}, {}, {}, {}
    ...
    for profession in train_activations.keys():
        ...
        if compute_wasserstein:
            # Computed BEFORE masking (on full activations); independent of select_top_k.
            wasserstein_dist = compute_mean_wasserstein_distance(train_acts, train_labels)
        else:
        ...
        if use_sklearn:
            probe, test_accuracy, test_f1 = train_sklearn_probe(
                train_acts,
                train_labels,
                test_acts,
                test_labels,
            ...
            probe, test_accuracy, test_f1 = train_probe_gpu(
                train_acts,
                train_labels,
                test_acts,
                test_labels,
        ...
        if compute_wasserstein:
            print(f"Test accuracy for {profession}: {test_accuracy}, F1: {test_f1}, Wasserstein: {wasserstein_dist}")
        else:
            print(f"Test accuracy for {profession}: {test_accuracy}, F1: {test_f1}")

        probes[profession] = probe
        test_accuracies[profession] = test_accuracy
        test_f1_scores[profession] = test_f1
        if compute_wasserstein:
            wasserstein_distances[profession] = wasserstein_dist

    return probes, test_accuracies, test_f1_scores, wasserstein_distances
