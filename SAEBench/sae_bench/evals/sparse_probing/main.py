...
def run_eval_single_dataset(
    ...
    if not os.path.exists(activations_path):
        ...
        llm_probes, llm_test_accuracies, llm_test_f1s, llm_wassersteins = probe_training.train_probe_on_activations(
            all_train_acts_BD,
            all_test_acts_BD,
            select_top_k=None,
            use_sklearn=False,
        ...
        llm_results = {
            "llm_test_accuracy": llm_test_accuracies,
            "llm_test_f1": llm_test_f1s,
            "llm_wasserstein": llm_wassersteins,
        }

        for k in config.k_values:
            # Wasserstein is independent of select_top_k (see probe_training.train_probe_on_activations),
            # so we only compute and report it for k=1; higher-k values would just duplicate the number.
            llm_top_k_probes, llm_top_k_test_accuracies, llm_top_k_f1s, llm_top_k_wassersteins = (
                probe_training.train_probe_on_activations(
                    all_train_acts_BD,
                    all_test_acts_BD,
                    select_top_k=k,
                    compute_wasserstein=(k == 1),
                )
            )
            llm_results[f"llm_top_{k}_test_accuracy"] = llm_top_k_test_accuracies
            llm_results[f"llm_top_{k}_test_f1"] = llm_top_k_f1s
            if k == 1:
                llm_results[f"llm_top_{k}_wasserstein"] = llm_top_k_wassersteins
    ...
    if not config.lower_vram_usage:
        ...
        # We use GPU here as sklearn.fit is slow on large input dimensions, all other probe training is done with sklearn.fit
        _, sae_test_accuracies, sae_test_f1s, sae_wassersteins = probe_training.train_probe_on_activations(
        ...
        per_class_results_dict["sae_test_f1"] = sae_test_f1s
        per_class_results_dict["sae_wasserstein"] = sae_wassersteins
    else:
        per_class_results_dict["sae_test_accuracy"] = {"-1": -1}
        per_class_results_dict["sae_test_f1"] = {"-1": -1}
        per_class_results_dict["sae_wasserstein"] = {"-1": -1}
    ...
    for k in config.k_values:
        # See note above: Wasserstein doesn't depend on k here, so only compute/record at k=1.
        sae_top_k_probes, sae_top_k_test_accuracies, sae_top_k_f1s, sae_top_k_wassersteins = (
            probe_training.train_probe_on_activations(
                all_sae_train_acts_BF,
                all_sae_test_acts_BF,
                select_top_k=k,
        ...
        per_class_results_dict[f"sae_top_{k}_test_f1"] = sae_top_k_f1s
        if k == 1:
            per_class_results_dict[f"sae_top_{k}_wasserstein"] = sae_top_k_wassersteins
