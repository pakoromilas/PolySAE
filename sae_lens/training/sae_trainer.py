...
class SAETrainer(Generic[T_TRAINING_SAE, T_TRAINING_SAE_CONFIG]):
    ...
    def _train_step(
        ...
        # Apply orthogonality retraction for PolySAE
        # This ensures U matrices span orthogonal subspaces after each update
        if hasattr(sae, "orthogonalize_U"):
            sae.orthogonalize_U()
    ...
    def _build_train_step_log_dict(
        ...
        # Log PolySAE lambda coefficients if present
        if hasattr(self.sae, "lambda2"):
            log_dict["polysae/lambda2"] = self.sae.lambda2.item()
        if hasattr(self.sae, "lambda3"):
            log_dict["polysae/lambda3"] = self.sae.lambda3.item()
