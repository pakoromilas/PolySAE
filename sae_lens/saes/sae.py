...
class SAE(HookedRootModule, Generic[T_SAE_CONFIG], ABC):
    ...
    def get_decoder_directions(self) -> torch.Tensor:
        """Return decoder directions, normalized to unit norm per feature.
        
        For linear decoders, this is W_dec normalized to unit row norms.
        Subclasses with non-linear decoders (e.g., PolySAE) should override this.
        
        Returns:
            Tensor of shape (n_features, d_model) = (d_sae, d_in) with unit-norm rows.
            Each row is the normalized decoder direction for a single feature.
            
        Contract:
            - Shape is always (d_sae, d_in) regardless of decoder type
            - Each row has L2 norm == 1.0 (within numerical precision)
            - For non-linear decoders, this represents the "linear directional view"
        """
        # W_dec is (d_sae, d_in) - each row is a feature's decoder direction
        directions = self.W_dec / self.W_dec.norm(dim=-1, keepdim=True).clamp(min=1e-8)
